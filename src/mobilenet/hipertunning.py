...
# Função para treinar o modelo com tqdm mostrando a porcentagem de conclusão
def train_model(model, criterion, optimizer, train_loader, val_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        # Inicializa a barra de progresso com o total de batches da época
        ...
        for i, (inputs, labels) in loop:
            ...
            # Estatísticas de perda e acurácia
            running_loss += loss.item()
            _, predicted = torch.max(outputs.data, 1)
            total += labels.size(0)
            correct += (predicted == labels).sum().item()

            # Cálculo da perda e acurácia a cada batch
            current_loss = running_loss / (i + 1)
            current_accuracy = 100 * correct / total

            # Atualiza a barra de progresso com a porcentagem de conclusão
            percent_complete = (i + 1) / len(train_loader) * 100
            loop.set_postfix(loss=current_loss, accuracy=current_accuracy, progress=f'{percent_complete:.2f}%')
        ...
        # Exibe os resultados ao final da época
...
def objective(trial):
    ...
    n_units_fc1 = trial.suggest_int('n_units_fc1', 512, 2048, step=256)
    
    train_loader = DataLoader(train_dataset, batch_size=batch_size, sampler=sampler)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)

    # Usar MobileNetV3-Large em vez de DenseNet
    ...
    # Modificar a última camada (classifier) para o número de classes (num_classes)
    model.classifier = nn.Sequential(
        nn.Linear(model.classifier[0].in_features, n_units_fc1), 
        nn.ReLU(inplace=True),
        nn.Dropout(dropout_rate),
        nn.Linear(n_units_fc1, num_classes)
    )
    ...
    results_path = "experiment_results_mobilenetv3_large.txt"
    
    with open(results_path, 'a') as f:
        f.write(f"Trial: {trial.number}, LR: {lr}, Optimizer: {optimizer_name}, Batch size: {batch_size}, "
                f"Dropout rate: {dropout_rate}, FC1 units: {n_units_fc1}, "
...
best_results_path = "best_experiment_result_mobilenetv3_large.txt"
...
best_model.classifier = nn.Sequential(
    nn.Linear(best_model.classifier[0].in_features, best_trial.params['n_units_fc1']),
    nn.ReLU(inplace=True),
    nn.Dropout(best_trial.params['dropout_rate']),
    nn.Linear(best_trial.params['n_units_fc1'], num_classes)
)
